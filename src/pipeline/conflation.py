...
def _overlap(
    ...
    the feature the way covers as a pair of fractions). The span is what makes
    exclusivity work along a corridor rather than across it.
    ...
    return fraction, (min(positions), max(positions)) if positions else None
...
def conflate(
    ...
    """
    candidates: list[tuple[float, int, AgencyFeature, tuple[float, float] | None]] = []
    ...
    for way_id, coordinates in ways:
        ...
        for feature in features:
            ...
            if min(delta, 180.0 - delta) > bearing_tolerance_deg:
                continue
            overlap, span = _overlap(coordinates, feature.coordinates, max_separation_m)
            if overlap < min_overlap:
                continue
            candidates.append((overlap, way_id, feature, span))

    # Best first: precedence, then overlap.
    def rank(candidate: tuple[float, int, AgencyFeature, tuple[float, float] | None]):
        overlap, _, feature, _span = candidate
        try:
    ...
    for overlap, way_id, feature, span in sorted(candidates, key=rank):
        match = Match(
            osm_way_id=way_id,
