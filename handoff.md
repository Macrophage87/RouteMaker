...
| Test quality | `sample_cycle_lane`'s "one and the same way" guard is deletable with the suite green; the covering test's edges also disagree about the lane. | **Closed (T5, 0a765a2).** `test_two_ways_that_agree_about_the_lane_still_answer_nothing` has edges that agree about the lane and differ only in way id; the mutation `if None in way_ids` fails it and only it. |
...
|---|---|
| Routing (17/17 caught) | Row 65, plus: `sample_cycle_lane` filters to the sentinel's way, refuses an unidentified edge and disagreeing edges and reads `none` as no lane - as of T5, agreement is checked over every edge on the way, a missing lane counting as `none`, before `none` is read as no lane (the way id itself is a §7 row until a real rebuild names it); an approved override of an unhandled kind, and an `OverrideRefused`, stop the rebuild as `ValidationFailed` rather than being retried five times; a `valhalla_exception_t` body on a failed trace read is recognised and terminal; `-v` pinned; a sentence beside `VALHALLA_CONFIG_DIR` on the baked configs and the bind-mounted Lua. |
...
| **The tier-1 sentinel's own way id is not recorded, so the derived-tag check is narrowed to "one way" rather than to *that* way.** VALIDATE reads a known tier-1 block back out of the standard variant's tiles and asks what cycle lane it carries. The read now refuses a trace spanning more than one way, an edge with no way id, edges that disagree and (as of T5) a half-transformed block, where a lane is mixed with `none` or with an edge carrying no lane, and only an agreed `none` is read as no lane — but `pipeline/run.py`'s `DERIVED_SENTINEL_WAY_ID` is `None`, because the sentinel is a pair of coordinates picked off a map (`REBUILD_SENTINEL_TIER1_EDGE`, itself recorded as unconfirmed) and nothing here can honestly assert which OSM way they lie on. | PLAN.md:71 | Closed by the first real rebuild, which either matches the block or names the way it matched; the id then goes beside the coordinates. |
| ~~**Routing SF-1 (round 10): a half-transformed sentinel block answered `separated`.**~~ `sample_cycle_lane` filtered `none` and missing lanes out before checking that the way's edges agreed, so a block the transform reached half of (one edge `separated`, the next `none` or silent) passed VALIDATE. | PLAN.md:71 | **Closed (T5, f538961).** Agreement is taken over every edge on the way, a missing lane counting as `none`, and only then is `none` read as no lane. Covered by `test_a_half_transformed_block_answers_nothing`. |
| **The sentinel read's agreement check has never run against real Valhalla tiles.** T5's check is exercised only through the fake binaries: there is no Valhalla binary or reference data on this host, and A4 is interactive. Also unverified: whether real `trace_attributes` leaves `cycle_lane` out for kNone edges or always sends it. The code treats a missing lane as `none`, which is safe either way. | PLAN.md:71 | Recorded, not fixed. Closed by the same first real rebuild as the way-id row above. |
| **T5 residual: the half-transformed test does not catch a weakened agreement check on every hash seed.** With `if len(lanes) != 1:` deleted or weakened to `if not lanes:`, `test_a_half_transformed_block_answers_nothing` fails only when `set.pop()` on `{'separated', 'none'}` returns `separated`, which depends on `PYTHONHASHSEED` (seed 3 of 0-4 here). The mutant is still caught on every seed by the disagreeing-edges case in `test_a_cycle_lane_read_that_cannot_be_attributed_to_one_way_answers_nothing`, and the T5 report disclosed this. What the half-transformed test guards reliably is the normalisation (missing lane counts as `none`), and its docstring's "disagreement is the only reason left to refuse" overstates it on some seeds. | - | Open (test quality). Make the case order-independent, for example a third distinct lane or an assertion no `pop` order can satisfy, or say in the docstring that the sibling test guards the agreement check. |
