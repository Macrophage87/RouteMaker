...
(249 commits, all pushed — 248 plus this wave-7 merge) · **Suite:** 2988 tests, green

**Phase 1 is not accepted.** It has been through eight rounds of independent review. Round 8
returned **one ACCEPT** — the cycling / DC-region domain, for the third round running — alongside
five REVISE, each carrying a single blocking finding except the database area, which carried two.
None of round 8's blockers was a regression from a fix wave: each had been in the tree since before
wave 1 and was reached by a reviewer told to look where the earlier rounds had not. Every round-3
through round-8 finding is closed on this tree, each fix carrying a mutation confirmed to fail, and
each panel has verified the previous round's findings closed by reverting the fix rather than by
reading. **A round-9 panel has not run.** One area of six is accepted; **nobody has accepted the
phase**, and the next step is that panel.

The blocker count per round has been 10, 14, 9, 5, 6 across rounds 4 to 8 (round 8's six are five
distinct findings; the map-widget one was filed twice). The findings are getting smaller — round 4's
was "nothing produces the source extract", round 8's include a healthcheck's container and a widget's
default layer — but acceptance needs six reviewers to report zero blockers in the same round, and
with a residual defect rate that is low but not zero, one reviewer finding one real item remains the
likely outcome for a while. Whether a pre-existing, non-regression find should keep withholding
acceptance, or be recorded in §7 and fixed in the next wave without doing so, is the owner's call;
the loop as run holds the strict rule.
...
Recorded rather than left for a round-9 reviewer to discover. None of these is a defect in shipped
...
| **The free-space alert measures whichever filesystem the container it runs in resolves `TILES_DIR` on.** `check_operations`' fourth check is a `statvfs` on `settings.TILES_DIR`, or on its nearest existing ancestor — which is what `pipeline.tiles.check_disk_gate` effectively measures, since it creates the directory first. In `api`, the container the documented cron entry uses, no part of the data volume is mounted and `DATA_ROOT` falls back to `BASE_DIR / "data"`, so the number is about the container's own writable layer. The line names the path it read, which makes that visible rather than silent, but it does not make it right: the honest home for the check is `rebuild` or `worker`. | PLAN.md:290 (the 80 % disk alert) | Built and honest about what it measured; the cron entry's container is the deploy owner's call. |
...
`docs/review/round-7/` and `docs/review/round-8/`, one per reviewer, each directory's README carrying the verdict table and
...
reports, rows 54–58 from round 7's six, rows 59–64 from round 8's six).
...
**The next step is the round-9 panel**, on this tree, with the same **six reviewers**. One of the
six accepted in round 8 — the domain, for the third time — and wave 7 built its should-fixes anyway,
one of which (`graph.lua:69`) was the sign that decides whether a river crossing exists in the graph,
with no test on either side of the suite. Round 8's blockers were all pre-existing and all found by
reaching for new surface; the header records what that means for convergence.
