...
| **A4** First rebuild | `run_rebuild_now` in the `rebuild` container, then polls the run row every minute up to eight hours (`--rebuild-timeout`). On a fresh host it follows the runbook's two-phase first rebuild: the first run downloads the extract and stops at the reference-data stage, the script prints step 6 of "First rebuild on a fresh host" (the three GeoJSON inputs and the install command), waits, re-checks the installer, and fires again. Requires the final run to have succeeded, `tiles/<variant>/current` to point at a build carrying `tiles.tar` on all three variants, restarts the three routers, sends a canary bicycle route (Lincoln Memorial to Union Station) to each router from inside the network, and runs `check_operations`. | A weekly rebuild completes against the real Valhalla and each variant serves routes. |
